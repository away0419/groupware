package com.it.groupware.controller;
...
import com.it.groupware.common.ExcelReadOption;
import com.it.groupware.common.ExcelUtil;
...
public class ExcelUploadController {
	...
	public int excelUploadInsert(MultipartHttpServletRequest multiRequest){
		...
		List<AddBookFolDTO> addBookFolList = addBookFolService.selectFol(empNo); // 주소록 목록 리스트
		int addBookFolNameDefaultCheck=0; // 주소록 목록명에 기본이 있는지 체크
		int addBookFolNameCheck=0;
		
		
		
		for(AddBookFolDTO vo : addBookFolList) {
		...
		if(addBookFolNameDefaultCheck == 0) { // 0이면 주소록 목록에 기본 추가
			AddBookFolDTO vo = new AddBookFolDTO();
			vo.setEmpNo(empNo);
		...
		for(int i=0; i<excelContent.size(); i++) { // 엑셀 정보 만큼 반복
			...
			addBookDTO.setAddressBookName(excelContent.get(i).get("A"));
			addBookDTO.setAddressBookTel(excelContent.get(i).get("B"));
			addBookDTO.setAddressBookMaile(excelContent.get(i).get("C"));
			addBookDTO.setAddressBookPosition(excelContent.get(i).get("D"));
			addBookDTO.setAddressBookOfficeName(excelContent.get(i).get("E"));
			addBookDTO.setAddressBookOfficeTel(excelContent.get(i).get("F"));
			...
			for(AddBookFolDTO vo : addBookFolList) {// 주소록 목록만큼 반복
				if(vo.getAddressFolderName() != null && !vo.getAddressFolderName().isEmpty()) {
					if(vo.getAddressFolderName().equals(addBookFolName)) {// 만약 카테고리명이 같다면 카테고리넘버 셋팅, 네임체크 추가 후 탈출
						addBookDTO.setAddressFolderNo(vo.getAddressFolderNo());
						addBookFolNameCheck++;
						break;
					}
				}
			...
			if(addBookFolNameCheck==0) {// 만약 0일경우 카테고리 추가 후 카테고리 넘버 셋팅
				AddBookFolDTO folvo = new AddBookFolDTO();
			...
			if(excelContent.get(i).get("H") != null && !excelContent.get(i).get("H").isEmpty()) {
				if(excelContent.get(i).get("H").equals("1") || excelContent.get(i).get("H").equals("남") 
					...
					addBookDTO.setAddressBookGender("남자");
			...
			addBookService.insertAddBook(addBookDTO);
			insertCnt++;
