<template>
  <div class="card shadow mb-4">
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary">반려된 문서 목록</h6>
    </div>
    <div class="card-body">
      <div class="table-responsive">
        <table class="table table-bordered table-hover">
          ...
          <tbody>
            <tr v-if="returnSendList == ''">
              <td colspan="3" align="center">최근 항목이 없습니다.</td>
            </tr>
            <tr v-else v-for="(item, index) in returnSendList" :key="index">
    ...
    sendList(
      {
        currentPage: 1,
        perPage: 5,
        electronicCompletFlag: 2,
        empNo: this.empInfo.empNo,
        electronicDraft: 0,
      },
      (res) => {
        this.returnSendList = res.data.list;
      },
      () => {},
    );
  },
