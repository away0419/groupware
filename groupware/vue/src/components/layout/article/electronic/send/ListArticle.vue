...
<script>
import { sendList, selectElSty } from "@/api/electronic";
...
export default {
  ...
  created() {
    ...
    } else if (this.listTitle == "temp") {
      this.titleNo = 3;
      this.titleName = "임시 저장 목록";
  ...
  methods: {
    async myProvider() {
      ...
      if (this.titleNo == 0) {
      ...
      } else if (this.titleNo == 1) {
        params.electronicCompletFlag = 1;
        params.electronicDraft = 0;
        ...
        params.electronicCompletFlag = 0;
        params.electronicDraft = 1;
        await sendList(
          params,
          (res) => {
            this.items = res.data.list;
    ...
    async rowClick(item) {
      if (this.titleNo != 3) {
        ...
        this.$router.push({
          ...
          params: {
            electronicNo: item.electronicNo,
            electronicDratf: item.electronicDraft,
            folderNo: elSty.folderNo,
            folderStyleNo: elSty.folderStyleNo,
          },
        });
      }
    },
